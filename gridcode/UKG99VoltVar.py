...
class UKG99VoltVar:
    ...
    def __init__(self, characteristic_type, power_level, eut) -> None:
        
        self.characteristic = characteristic_type
        self.eut = eut

        if characteristic_type == 1:
            self.Q = np.array([1, 0, 0, -1]) *eut.qmax * power_level
            self.V = np.array([0.95, 1, 1, 1.05]) * eut.vn

        elif characteristic_type == 2:
            self.Q = np.array([0.5, 0, 0, -0.5]) *eut.qmax * power_level
            self.V = np.array([0.9, 0.98, 1.02, 1.1]) * eut.vn

        elif characteristic_type == 3:
            self.Q = np.array([0.05, 0, 0, -0.05]) *eut.qmax * power_level
